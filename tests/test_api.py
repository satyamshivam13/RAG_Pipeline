...
class TestQueryEndpoint:
    ...
    def test_query_basic(self, client):
        ...
        ingest_payload = {
            "texts": ["RAG systems combine retrieval and generation."],
            "source": "test"
        }
    ...
    def test_query_streaming_response(self, client):
        ...
        # First ingest some data
        ingest_payload = {
            "texts": ["RAG systems work by retrieving and generating."],
            "source": "test"
        }
        client.post("/ingest", json=ingest_payload)
...
class TestResponseModels:
    ...
    def test_query_response_structure(self, client):
        ...
        # Validate expected fields
        required_fields = {
            "query", "answer", "is_reliable", "consistency_score",
            "retrieved_chunks", "processing_time_ms", "evaluation_status"
        }
        assert required_fields.issubset(data.keys())
    ...
    def test_retrieved_chunk_structure(self, client):
        ...
        ingest_payload = {
            "texts": ["Test document for retrieval"],
            "source": "test"
        }
        client.post("/ingest", json=ingest_payload)
